...
class UserTest < ActiveSupport::TestCase
  ...
  test '当日0時以降に作成した投稿数を返す' do
    ...
    travel_to Time.zone.local(2026, 8, 15, 12) do
      posts(:alice_first).update_columns(created_at: Time.current.beginning_of_day) # rubocop:disable Rails/SkipsModelValidations
      posts(:alice_second).update_columns(created_at: Time.current.beginning_of_day - 1.second) # rubocop:disable Rails/SkipsModelValidations
